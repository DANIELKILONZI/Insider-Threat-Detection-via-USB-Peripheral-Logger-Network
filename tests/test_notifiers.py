...
def test_send_email_noop_when_not_configured(monkeypatch):
    monkeypatch.setattr("server.notifiers.SMTP_HOST", "")
    import server.notifiers as n
    # Should not raise
...
def test_send_email_noop_when_no_recipients(monkeypatch):
    monkeypatch.setattr("server.notifiers.SMTP_HOST", "smtp.example.com")
    monkeypatch.setattr("server.notifiers.SMTP_TO", "")
    import server.notifiers as n
    n.send_email_alert(_alert())
...
def test_send_email_calls_smtp(monkeypatch):
    monkeypatch.setattr("server.notifiers.SMTP_HOST", "smtp.example.com")
    monkeypatch.setattr("server.notifiers.SMTP_PORT", 587)
    monkeypatch.setattr("server.notifiers.SMTP_FROM", "itdn@example.com")
    monkeypatch.setattr("server.notifiers.SMTP_TO", "soc@example.com")
    monkeypatch.setattr("server.notifiers.SMTP_USE_TLS", True)
    monkeypatch.setattr("server.notifiers.SMTP_USER", "")
    monkeypatch.setattr("server.notifiers.SMTP_PASSWORD", "")
    monkeypatch.setattr("server.notifiers.NOTIFY_MIN_SEVERITY", "LOW")
    ...
    with patch("server.notifiers.smtplib.SMTP", mock_smtp):
...
def test_send_email_handles_smtp_error(monkeypatch):
    ...
    monkeypatch.setattr("server.notifiers.SMTP_FROM", "itdn@example.com")
    monkeypatch.setattr("server.notifiers.SMTP_TO", "soc@example.com")
    monkeypatch.setattr("server.notifiers.SMTP_USE_TLS", False)
    monkeypatch.setattr("server.notifiers.SMTP_USER", "")
    monkeypatch.setattr("server.notifiers.SMTP_PASSWORD", "")
    monkeypatch.setattr("server.notifiers.NOTIFY_MIN_SEVERITY", "LOW")
    ...
    with patch("server.notifiers.smtplib.SMTP", side_effect=smtplib.SMTPException("conn refused")):
...
def test_send_slack_noop_when_not_configured(monkeypatch):
    monkeypatch.setattr("server.notifiers.SLACK_WEBHOOK_URL", "")
    import server.notifiers as n
...
def test_send_slack_posts_webhook(monkeypatch):
    monkeypatch.setattr("server.notifiers.SLACK_WEBHOOK_URL", "https://hooks.slack.com/test")
    monkeypatch.setattr("server.notifiers.NOTIFY_MIN_SEVERITY", "LOW")
    ...
    with _patch("server.notifiers.urllib.request.urlopen", return_value=mock_resp) as mock_open:
